...
class WebPolicyScraper:
    ...
    def _is_quality_content(self, text: str) -> bool:
        ...
        junk_indicators = [
            ...
            'terms of service', 'cookie policy', 'javascript', 'enable cookies',
            'browser not supported', 'error 404', 'page not found'
        ]
        ...
        useful_keywords = [
            'shipping', 'delivery', 'return', 'refund', 'warranty', 'policy',
            'customer', 'service', 'support', 'order', 'product', 'payment'
        ]
    ...
    def _extract_contextual_content(self, soup: BeautifulSoup, selectors: str, keywords: List[str], max_items: int = 3) -> List[str]:
    ...
    def _scrape_return_policies(self) -> List[Dict[str, Any]]:
        ...
        return_keywords = ['return', 'refund', 'exchange', 'policy', 'purchase', 'buy', 'shop', 'customer']
        ...
        for target in targets:
            try:
                ...
                policy_texts = self._extract_contextual_content(soup, target['selectors'], return_keywords)
    ...
    def _scrape_shipping_policies(self) -> List[Dict[str, Any]]:
        ...
        shipping_keywords = ['shipping', 'delivery', 'ship', 'freight', 'dispatch', 'send', 'mail', 'transport']
        ...
        for target in targets:
            try:
                ...
                soup = BeautifulSoup(response.text, 'html.parser')
                shipping_texts = self._extract_contextual_content(soup, target['selectors'], shipping_keywords)
    ...
    def _scrape_warranty_information(self) -> List[Dict[str, Any]]:
        ...
        warranty_keywords = ['warranty', 'guarantee', 'protection', 'coverage', 'defect', 'repair', 'replace', 'quality']
        ...
        for target in targets:
            try:
                ...
                soup = BeautifulSoup(response.text, 'html.parser')
                warranty_texts = self._extract_contextual_content(soup, target['selectors'], warranty_keywords)
    ...
    def _scrape_additional_policies(self) -> List[Dict[str, Any]]:
        ...
        for source in additional_sources:
            try:
                ...
                if response:
                    soup = BeautifulSoup(response.text, 'html.parser')
                    relevant_texts = self._extract_contextual_content(soup, 'p, li, td', source['keywords'], max_items=2)
...
class ProductPolicyScraper:
    ...
    def extract_policies(self) -> List[Dict[str, Any]]:
        ...
        for warranty, count in warranty_info.items():
            support_docs.append({
                "title": f"Product Warranty Information",
        ...
        for shipping, count in shipping_info.items():
            support_docs.append({
                "title": f"Product Shipping Information",
    ...
    def get_policy_summary(self) -> Dict[str, Any]:
        ...
        warranties = [p.get('warrantyInformation', '') for p in products if p.get('warrantyInformation')]
        shipping = [p.get('shippingInformation', '') for p in products if p.get('shippingInformation')]
...
class KnowledgeProvider:
    ...
    def _deduplicate_and_enrich(self, knowledge_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def get_knowledge_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_web_scraped_content": any('scraped_' in item.get('source', '') for item in knowledge),

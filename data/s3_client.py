...
class ProductS3Client:
    ...
    def load_products(self, force_refresh: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            # Try to load from S3
            try:
                response = self.s3_client.get_object(Bucket=self.bucket_name, Key=self.products_key)
                ...
                if os.path.exists(local_path):
                    ...
                    try:
                        with open(local_path, "r") as f:
                            ...
                            logger.info(
                                f"✅ Loaded {len(self.cached_products)} products from local file"
                            )
    ...
    def get_last_modified(self) -> Optional[str]:
        """Get last modified timestamp of products in S3"""
        try:
            response = self.s3_client.head_object(Bucket=self.bucket_name, Key=self.products_key)
            return response["LastModified"].isoformat()
        except ClientError:
            logger.warning("Could not get last modified timestamp for products")
            return None
        except Exception as e:
    ...
    def get_product_stats(self) -> Dict[str, Any]:
        ...
        stats = {"products_available": True, "cached": self.cached_products is not None}
...
class SupportS3Client:
    ...
    def __init__(self, s3_client, bucket_name: str):
        self.s3_client = s3_client
        self.bucket_name = bucket_name
        self.support_knowledge_key = os.getenv(
    ...
    def load_support_data(self, use_cache: bool = True) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = self.s3_client.get_object(
                Bucket=self.bucket_name, Key=self.support_knowledge_key
            )
            ...
            logger.info(
                f"✅ Loaded support data from S3: {support_data['metadata']['total_documents']} documents"
            )
            return support_data
    ...
    def upload_support_data(
        self, support_data: Dict[str, Any] = None, create_backup: bool = True
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✅ Successfully uploaded support data to S3: {self.support_knowledge_key}"
            )
    ...
    def generate_support_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            from support_docs.FAQ_Knowledge_base import KnowledgeProvider, ProductPolicyScraper
            ...
            support_data = {
                "metadata": {
                    ...
                    "version": "1.0",
                    "categories": sorted(
                        list(set([doc.get("category", "") for doc in all_support_docs]))
                    ),
                    "document_types": sorted(
                        list(set([doc.get("type", "") for doc in all_support_docs]))
                    ),
                    "sources": sorted(
                        list(set([doc.get("source", "") for doc in all_support_docs]))
                    ),
                },
    ...
    def get_support_stats(self) -> Dict[str, Any]:
        ...
        stats = {"support_available": True, "cached": self.cached_support_data is not None}
...
class UnifiedS3Client:
    ...
    def _init_s3_client(self):
        ...
        try:
            self.s3_client = boto3.client(
                ...
                config=Config(read_timeout=10, connect_timeout=5),
            )
    ...
    def get_products_last_modified(self) -> Optional[str]:
        """Get last modified timestamp of products in S3 - backward compatible method"""
        return self.product_client.get_last_modified()

    # Unified data operations
    def upload_data(
        self, data_type: str, file_path: str = None, data: Any = None, create_backup: bool = True
    ) -> bool:

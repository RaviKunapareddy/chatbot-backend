...
class ConversationMemory:
    ...
    def _init_redis(self):
        ...
        try:
            ...
            if not redis_url:
                ...
                # Debug prints
                print(f"Connecting to Redis with: redis://{username}:***@{host}:{port}/{db}")
                print(f"Raw password: [{password}]")
                print(f"Encoded password: [{encoded_pw}]")
                redis_url = f"redis://{username}:{encoded_pw}@{host}:{port}/{db}"
            else:
                print(f"Connecting to Redis with REDIS_URL: {redis_url[:30]}... (password masked)")
            ...
            print(f"❌ Redis connection failed: {e.__class__.__name__}: {e}")
            traceback.print_exc()
    ...
    def get_context(self, session_id: str) -> str:
        ...
        # Redis implementation only
        conv_key = f"{self.conversation_prefix}{session_id}"
        conversations = self.redis_client.lrange(conv_key, -3, -1)  # Last 3 exchanges
    ...
    def cleanup_inactive_sessions(self, timeout_minutes: int = 30):
        ...
        print("✅ Redis TTL handles session cleanup automatically")
